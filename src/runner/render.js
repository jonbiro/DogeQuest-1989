...
export function createView(canvas) {
  ...
  // Destination kits own each landscape's look. The renderer keeps placement
  // and the seeded random stream; shoulder counts and spacing come from the
  // kit so a half-authored destination fails fast instead of shipping quietly.
  const kitHelpers = {box, ball, cone, mesh, trunkGeometry, palmFrondGeometry, featheredPalmGeometry, mushroomCapGeometry, addBambooLeaves, random};
  for (const kit of DESTINATION_KITS) for (let i = 0; i < kit.roles.shoulder.count; i++) {
    const area = kit.area;
    const group = new THREE.Group();
    const side = i % 2 ? 1 : -1;
    ...
    Object.assign(group.userData, {
      area,
      offset: i * kit.roles.shoulder.spacing + kit.roles.shoulder.offset,
      variant: 2,
    });
    buildShoulderFamily(area, group, kitHelpers);
    scenery.add(group);
    decorations.push(group);
  }
  ...
  for (let area = 0; area < AREAS.length; area++) for (let variant = 0; variant < 4; variant++) {
    ...
    buildSignature(area, group, variant, kitHelpers);
    scenery.add(group);
    decorations.push(group);
  }
  ...
  for (let area = 0; area < AREAS.length; area++) for (let i = 0; i < 8; i++) {
    ...
    buildTrailMotif(area, group, kitHelpers);
    scenery.add(group);
    decorations.push(group);
  }
