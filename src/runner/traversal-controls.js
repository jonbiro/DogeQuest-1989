...
export function traversalDescription(run){
  if(run.raft)return 'River raft. Tap LEFT or RIGHT, or swipe one lane at a time, to steer between rocks. Lift after a move, or pause with your finger down before swiping again. Jump and slide return at the shore. Escape pauses.';
  if(run.zipline)return 'Zipline ride. Tap LEFT or RIGHT, or swipe one lane at a time, to collect bones. Lift after a move, or pause with your finger down before swiping again. Jump and slide return after the cable. Escape pauses.';
  return '3D running trail. The buttons are easiest: tap LEFT or RIGHT for one lane, JUMP for a log or gap, and SLIDE for an overhead gate. Swipes are optional: one swipe moves one lane. Lift after a move, or pause with your finger down before dragging again. A long drag still stops after one lane. A clear swipe can switch between steering and jump/slide without lifting. On touch screens, tap a left or right edge to steer or the center to jump. Arrow keys also work. Escape pauses.';
}
