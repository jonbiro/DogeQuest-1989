...
export function puppyPose(time,distance,{menu=false,reducedMotion=false,airborne=false,sliding=false,ziplining=false,rafting=false,skiing=false}={}) {
  ...
  return {
    ...
    legs:[0,1,2,3].map(i=>menu?0:rafting?(i%2===0?-.15:.18):ziplining?(i%2===0?-2.65:.25):airborne?(i%2===0?-.65:.5):sliding?-.9:skiing?(i%2===0?-.42:.42):Math.sin(distance*.82+(i===0||i===3?0:Math.PI))*.7),
  };
}
