...
export function puppyPose(time,distance,{menu=false,reducedMotion=false,airborne=false,sliding=false}={}) {
  ...
  return {
    ...
    legs:[0,1,2,3].map(i=>menu?0:airborne?(i<2?-.65:.5):sliding?-.9:Math.sin(distance*.82+(i===0||i===3?0:Math.PI))*.7),
  };
}
