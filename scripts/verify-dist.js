...
async function assertNoSymlinks(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (stats.isSymbolicLink()) {
      throw new Error(`Refusing to upload symbolic link: ${path.relative(projectRoot, entryPath)}`);
    }
...
export async function verifyDist() {
  ...
  if (!distStats.isDirectory()) throw new Error('The Pages artifact must be a directory.');

  const actualEntries = (await readdir(distDirectory)).sort();
  const expectedEntries = [...allowedEntries].sort();
  if (JSON.stringify(actualEntries) !== JSON.stringify(expectedEntries)) {
    throw new Error(`Unexpected Pages artifact entries: ${actualEntries.join(', ')}`);
  }
