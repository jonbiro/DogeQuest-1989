...
test("new and old saves honor system motion preference and keep sound opt-in",()=>{
  for(const value of [undefined,null,5,"bad",{}]) {
    assert.deepEqual(preferencesFrom(value,true),{sound:false,swipeOnly:false,reducedMotion:true,tiltSensitivity:'balanced'});
    assert.deepEqual(preferencesFrom(value,false),{sound:false,swipeOnly:false,reducedMotion:false,tiltSensitivity:'balanced'});
  }
});
test("explicit player preferences survive serialization and override system defaults",()=>{
  const value=JSON.parse(JSON.stringify({sound:true,swipeOnly:true,reducedMotion:false,tiltSensitivity:'steady'}));
  assert.deepEqual(preferencesFrom(value,true),value);
  assert.deepEqual(preferencesFrom({sound:"true",reducedMotion:0,swipeOnly:"true"},true),{sound:false,swipeOnly:false,reducedMotion:true,tiltSensitivity:'balanced'});
});
