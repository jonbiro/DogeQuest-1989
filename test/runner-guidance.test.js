...
test("touch onboarding teaches the safe opening without changing desktop guidance", () => {
  ...
  assert.equal(touchCoach(touchRun), "TAP OR DRAG · MOVE ONE LANE");
  touchRun.touchActionCount=1;
  assert.equal(touchCoach(touchRun), "ONE TAP = ONE MOVE · SWIPES OPTIONAL");
  touchRun.touchSwipeSeen=true;
  assert.equal(touchCoach(touchRun), "DRAG ONE LANE · KEEP FINGER DOWN");
  touchRun.touchOverdrag=true;
  assert.equal(touchCoach(touchRun), "ONE LANE AT A TIME · KEEP FINGER DOWN");
  touchRun.touchActionCount=2;
  assert.equal(touchCoach(touchRun), "ONE LANE AT A TIME · KEEP FINGER DOWN", "the held-drag hint remains until the next move acknowledges it");
  touchRun.touchOverdrag=false;
  assert.equal(touchCoach(touchRun), "DRAG ONE LANE · KEEP FINGER DOWN", "the coach stays available while a new player is still learning");
  ...
  assert.equal(touchCoach(touchRun), "TAP OR DRAG · MOVE ONE LANE", "simulation inputs do not spend the touch lesson");
...
test("live touch coaching explains the gesture where the thumb is held", () => {
  ...
  assert.equal(touchGestureCoach({pointerType:'touch',axis:'horizontal',laneDirection:'left'}), "← ONE LANE · KEEP FINGER DOWN");
  assert.equal(touchGestureCoach({pointerType:'touch',axis:'horizontal',laneDirection:'right'}), "→ ONE LANE · KEEP FINGER DOWN");
  assert.equal(touchGestureCoach({pointerType:'touch',axis:'vertical',laneDirection:'jump'}), "↑ JUMP · KEEP FINGER DOWN");
  assert.equal(touchGestureCoach({pointerType:'touch',axis:'vertical',laneDirection:'slide'}), "↓ SLIDE · KEEP FINGER DOWN");
});
