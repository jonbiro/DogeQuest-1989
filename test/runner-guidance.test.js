...
test("touch onboarding teaches the safe opening without changing desktop guidance", () => {
  ...
  assert.equal(touchCoach(touchRun), "TAP BUTTONS · ONE SWIPE = ONE MOVE");
  touchRun.touchActionCount=1;
  assert.equal(touchCoach(touchRun), "ONE TAP = ONE MOVE · SWIPES OPTIONAL");
  touchRun.touchSwipeSeen=true;
  assert.equal(touchCoach(touchRun), "ONE SWIPE = ONE MOVE · STOP, THEN DRAG AGAIN");
  touchRun.touchOverdrag=true;
  assert.equal(touchCoach(touchRun), "ONE SWIPE = ONE MOVE · STOP, THEN DRAG AGAIN");
  touchRun.touchActionCount=2;
  assert.equal(touchCoach(touchRun), "ONE SWIPE = ONE MOVE · STOP, THEN DRAG AGAIN", "the held-drag hint remains until the next move acknowledges it");
  touchRun.touchOverdrag=false;
  assert.equal(touchCoach(touchRun), "ONE SWIPE = ONE MOVE · STOP, THEN DRAG AGAIN", "the coach stays available while a new player is still learning");
  ...
  assert.equal(touchCoach(touchRun), "TAP BUTTONS · ONE SWIPE = ONE MOVE", "simulation inputs do not spend the touch lesson");
...
test("live touch coaching explains the gesture where the thumb is held", () => {
  ...
  assert.equal(touchGestureCoach({pointerType:'touch'}), "DRAG ONE WAY · ONE MOVE");
  assert.equal(touchGestureCoach({pointerType:'touch',axis:'horizontal',laneDirection:'left'}), "← ONE LANE · STOP, THEN DRAG AGAIN");
  assert.equal(touchGestureCoach({pointerType:'touch',axis:'horizontal',laneDirection:'right'}), "→ ONE LANE · STOP, THEN DRAG AGAIN");
  assert.equal(touchGestureCoach({pointerType:'touch',axis:'vertical',laneDirection:'jump'}), "↑ JUMP · STOP, THEN DRAG AGAIN");
  assert.equal(touchGestureCoach({pointerType:'touch',axis:'vertical',laneDirection:'slide'}), "↓ SLIDE · STOP, THEN DRAG AGAIN");
});
...
test("slide guidance renews only when the current slide will expire before impact", () => {
  ...
  assert.equal(actionCue(expiring), "↓ SLIDE AGAIN", "an expiring slide renews with its own prompt");
  expiring.slide=0;
