...
test('malformed, ambiguous and incompatible trail codes fall back to random play',()=>{
  for(const search of ['', '?trail=', '?trail=5-1','?trail=1--1','?trail=1-01','?trail=1-Z','?trail=1-zzzzzzz','?trail=1-0&trail=1-1','?trail=1-%3Cscript%3E'])
...
test('optional score targets round-trip without accepting ambiguous or malformed input',()=>{
  ...
  assert.equal(readTrailTarget('?trail=5-1j9&target=420'),0);
