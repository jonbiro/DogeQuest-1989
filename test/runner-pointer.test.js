...
test('a gradual cross-axis correction accumulates after a horizontal overdrag',()=>{
  const source=readFileSync(new URL('../src/runner/app.js',import.meta.url),'utf8');
  const start=source.indexOf('let pointer = null;');
  const end=source.indexOf('for (const button of document.querySelectorAll("[data-action]")',start);
  const handlers={},actions=[];
  ...
  handlers.pointerdown(touch);
  handlers.pointermove({...touch,clientX:82,timeStamp:120});
  // The overlong horizontal sample is safely bounded to one lane.
  handlers.pointermove({...touch,clientX:180,timeStamp:160});
  // A typical vertical correction arrives as several small samples, not one
  // 32px event. It should still become one jump without lifting.
  handlers.pointermove({...touch,clientX:180,clientY:112,timeStamp:180});
  handlers.pointermove({...touch,clientX:180,clientY:104,timeStamp:200});
...
test('a sparse release can finish a reverse segment without a final move event',()=>{
  ...
  handlers.pointerdown(touch);
  handlers.pointermove({...touch,clientX:82,timeStamp:120});
  // The browser gives us the return coordinate only with pointerup.
  handlers.pointerup({...touch,clientX:50,timeStamp:240});
  assert.deepEqual(actions,['right','left'],'release-only reverse travel is one clear move');
});

test('a sparse release can finish a vertical correction after a lane overdrag',()=>{
  const source=readFileSync(new URL('../src/runner/app.js',import.meta.url),'utf8');
  const start=source.indexOf('let pointer = null;');
  const end=source.indexOf('for (const button of document.querySelectorAll("[data-action]")',start);
  const handlers={},actions=[];
  ...
  handlers.pointermove({...touch,clientX:82,timeStamp:120});
  handlers.pointermove({...touch,clientX:180,timeStamp:160});
  // The final upward movement arrives only at release from a coalesced path.
  handlers.pointerup({...touch,clientX:180,clientY:80,timeStamp:240});
  assert.deepEqual(actions,['right','jump'],'release-only vertical intent is preserved');
});

test('coalesced touch samples preserve an out-and-back correction',()=>{
  const source=readFileSync(new URL('../src/runner/app.js',import.meta.url),'utf8');
  const start=source.indexOf('let pointer = null;');
  const end=source.indexOf('for (const button of document.querySelectorAll("[data-action]")',start);
  const handlers={},actions=[];
  ...
  handlers.pointerdown(touch);
  const samples=[
    {...touch,clientX:78,timeStamp:120},
    {...touch,clientX:110,timeStamp:150},
    {...touch,clientX:80,timeStamp:250},
  ];
  // Chrome can expose the complete thumb path through one coalesced event.
  handlers.pointermove({...touch,clientX:80,timeStamp:250,getCoalescedEvents:()=>samples});
