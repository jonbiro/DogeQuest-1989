...
test('portrait camp reserves separate pockets for copy, puppy and actions', () => {
  ...
  assert.match(layer, /#game\[data-state="menu"\] h1[\s\S]*?font-size:\s*clamp\(46px, 14vw, 58px\)/);
  assert.match(layer, /#game\[data-state="menu"\] h1[\s\S]*?white-space:\s*nowrap/);
  assert.match(layer, /#game\[data-state="menu"\] \.touch-launch-hint[\s\S]*?max-width:\s*222px/);
  assert.match(layer, /#game\[data-state="menu"\] \.start-actions[\s\S]*?margin-top:\s*10px/);
