...
for(const boosted of [false,true])test(`river tilt reactions preserve safe traversal, boost=${boosted}`,async()=>{
  ...
  assert.equal(run.rafts,1);assert.equal(run.hearts,3);assert.ok(run.bones>=9);
  tilt.stop();
});
