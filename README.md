...
- The on-screen buttons are the simplest touch controls: tap LEFT or RIGHT for one lane, JUMP for logs and gaps, and SLIDE under arches. Swipes are optional: press and hold on the trail, then drag one thumb-length at a time. Keep your finger down, stop moving briefly, then drag again for another lane move or lift whenever comfortable; quick over-swipes stop at one lane and oversized browser samples are rebased so one swipe cannot skip across the trail. A clear swipe can switch between steering and jump/slide without lifting. On touch screens, tapping a left or right edge is a one-lane fallback and a center tap jumps. Swipe up to jump logs and down to slide under arches.
