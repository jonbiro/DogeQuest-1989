...
- The on-screen buttons are the simplest touch controls: tap LEFT or RIGHT for one lane, JUMP for logs and gaps, and SLIDE under arches. Swipes are optional: press and hold on the trail, then drag left/right; one clear drag snaps one lane. Pause briefly, then drag again to cross another lane without lifting. A clear swipe can switch between steering and jump/slide without lifting. On touch screens, tapping a left or right edge is a one-lane fallback and a center tap jumps. Swipe up to jump logs and down to slide under arches.
