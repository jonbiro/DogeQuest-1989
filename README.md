...
- The on-screen buttons are the simplest touch controls: tap LEFT or RIGHT for one lane, JUMP for logs and gaps, and SLIDE under arches. Swipes are optional: press and hold on the trail, then drag a short way; one clear drag snaps one lane and a long drag stops safely at one lane. Pause or reverse before the next move, or lift and start another short drag. A clear swipe can switch between steering and jump/slide without lifting. On touch screens, tapping a left or right edge is a one-lane fallback and a center tap jumps. Swipe up to jump logs and down to slide under arches.
