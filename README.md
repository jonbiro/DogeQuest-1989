...
- Drag left/right to switch among three lanes; keep dragging to cross another lane without lifting, with one lane snapped per thumb segment. A clear swipe can switch between steering and jump/slide without lifting. On touch screens, tapping a left or right edge is a one-lane fallback and a center tap jumps. Swipe up to jump logs and down to slide under arches.
